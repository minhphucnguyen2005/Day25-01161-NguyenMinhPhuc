...
def _tokenize(text: str) -> list[str]:
    """Tokenize string into words and character 3-grams."""
...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        ...
        dot = sum(cnt * vec_b.get(tok, 0) for tok, cnt in vec_a.items())
        norm_a = math.sqrt(sum(cnt * cnt for cnt in vec_a.values()))
        norm_b = math.sqrt(sum(cnt * cnt for cnt in vec_b.values()))
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if exact_resp is not None:
            cached_q = self._redis.hget(exact_key, "query") or query
            if _looks_like_false_hit(query, cached_q):
                self.false_hit_log.append(
                    {
                        "query": query,
                        "cached_key": cached_q,
